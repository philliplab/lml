...
Non-linear relationship - try 1/x transformation:

```{r}
mod <- lm(1/medv ~ lstat, dat = Boston)
plot(1/medv ~ lstat, dat = Boston)
abline(mod)
par(mfrow=c(2,2))
plot(mod)
par(mfrow=c(1,1))
plot(hatvalues(mod) ~ 1/Boston$medv)
```
Much better, but now the error terms clearly depend on the value of the predictor. Might want to try a Bayesian model so that we can account for the increasing error?
